...
namespace deva {
  ...
  constexpr int log_dn(int x, int base) {
    return x == 0 ? -1 :
           x < base ? 0 :
           1 + log_dn(x/base, base);
  }

  inline int log2dn(unsigned int x) {
    return 8*sizeof(unsigned int)-1 - __builtin_clz(x);
  }
  inline int log2dn(unsigned long x) {
    return 8*sizeof(unsigned long)-1 - __builtin_clzl(x);
  }
  inline int log2dn(unsigned long long x) {
  ...
  template<typename T>
  int log2dn(T x, int when_zero) {
    return x == 0 ? when_zero : log2dn(x);
  }
  template<typename T>
  int log2up(T x, int when_zero) {
